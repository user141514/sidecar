...
test('collector follows only explicit watchdog continuation ancestry', async t => {
  ...
  for (const source of ['watchdog', 'coordinator']) {
    ...
    const authoritative = {
      contractVersion: 1, conversationId: 'conv_test', target, stateVersion: 1,
    ...
    const controller = new WorkController({ ledger, conversationHost: { read: async () => conversation, state: async () => authoritative }, watchdog: { completion: async () => ({ completed: true, result: 'done' }), ackCompletion: async () => true } })
