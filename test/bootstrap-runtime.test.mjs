...
test('bootstrap repairs owned Native Messaging registration on rerun', async (t) => {
  ...
  f.deps.readActiveRegistration = async () => ({
    ...
    hostPath: join(f.runtimeHome, 'bin', process.platform === 'win32' ? 'conversation-sidecar-host.bat' : 'conversation-sidecar-host'),
    dataRoot: null
  })
  ...
  assert.equal(f.calls.installs[0].hostPath, join(f.runtimeHome, 'bin', process.platform === 'win32' ? 'conversation-sidecar-host.bat' : 'conversation-sidecar-host'))
})
