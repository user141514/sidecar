export const CONVERSATION_TOOLS = [
  ...
  { name: 'conversation_send', description: 'Submit one prompt; return after acceptance while monitoring continues.', inputSchema: { type: 'object', properties: { conversation_id: { type: 'string' }, text: { type: 'string' }, app: { type: 'string' }, request_id: { type: 'string', minLength: 1, maxLength: 256 } }, required: ['conversation_id', 'text'], additionalProperties: false } },
...
export function dispatchConversationTool(host, name, args = {}) {
  ...
  if (name === 'conversation_send') {
    ...
    if (args.request_id !== undefined && (typeof args.request_id !== 'string' || !args.request_id || args.request_id.length > 256)) throw new TypeError('invalid request_id')
    return host.send(args.conversation_id, args.text, { app: args.app, ...(args.request_id ? { requestId: args.request_id } : {}) })
  }
